...
async def get_results(interaction: discord.Interaction):
    ...
    if role is None:
        await interaction.followup.send(
            "no role named *Professional* in channel named *professionals-signups*"
        )
        return
    messages = [
        message async for message in channel.history(limit=100, oldest_first=False)
    ]
    message = None
    for m in messages:
    ...
    if message is None:
        await interaction.followup.send(
            "no role named *Professional* mentioned in channel named *professionals-signups*"
        )
        return
    ...
    for reaction in message.reactions:
        async for user in reaction.users():
            if reaction.emoji == "🇵":
                participants[
                    str(interaction.guild.get_member(user.id).display_name).lstrip(
    ...
    if len(results) == 0:
        await interaction.followup.send(
            "Unfortunately I could not find any participants who qualified to win."
        )
        return
    ...
    participant_list = "\n".join(
        [
            "\n".join(
                ...
                if user.designation == "p"
            ),
            "\n".join(
                f"Rank ???: {user} - **???**"
                for user in participants
                if participants[user] == "false" and user not in coaches
            ),
        ]
    )
    coach_list = "\n".join(
        [
            "\n".join(
                build_score_message(user)
                for user in results.values()
                if user.designation == "c"
            ),
            "\n".join(
                f"Rank ???: *{user}* - **???**"
                for user in coaches
                if coaches[user] == "false"
            ),
        ]
    )
    ...
    if winner is not None:
        try:
            winner_discord_id = next(
                filter(
                    lambda member: member.nick == winner.name, interaction.guild.members
                )
            ).id
...
async def getResultsOnPage(results, x, fcm, participants, coaches):
    soup = BeautifulSoup(
        requests.get(
            f"https://na.finalfantasyxiv.com/lodestone/ranking/gc/weekly/?page={x}&filter=1"
            "&worldname=Siren"
        ).content,
        "html.parser",
    )
    ...
    for result in ranked_peeps:
        ...
        if player_id in fcm:
            ...
            ranking = int(
                str(result.select(".ranking-character__number")[0].text).strip()
            )
            designation = "x"
            win = False
            ...
            if player_id in results:
                results[player_id] = Scorer(
                    player_id,
                    name,
                    score + results.get(player_id).score,
                    results.get(player_id).ranking,
                    designation,
                    win,
                    True,
                )
